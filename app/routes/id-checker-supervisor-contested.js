module.exports = function (router) {
    ...
    router.get('/id-checker/v5/id-checker-supervisor-contested', function (req, res) {
        // pull in the url parameter
        var state = req.param('state')
        var queue = req.param('queue')
        var holding = req.param('holding')
        ...
        var devmode = req.param('devMode')
        var noabort = req.param('noabort')
        res.render('id-checker/v5/id-checker-supervisor-contested', {state: state, emailLink: emailLink, role: role, result: result, demographics: demographics, devMode: devmode, queue: queue, holding: holding, noabort: noabort}, function(err, html) {
            res.send(html)
        })
    })
}
