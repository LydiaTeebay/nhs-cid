module.exports = function (router) {
  router.get('/emails/email-forgotten-password', function (req, res) {
    ...
    router.get('/emails/v4/email-forgotten-password', function (req, res) {
        ...
        var serviceName = req.param('serviceName')
        // re-render the page along with the parameter
        res.render('emails/v4/email-forgotten-password', {reason: theReason, vouch: vouched, service: service, serviceName: serviceName}, function(err, html) {
            res.send(html)
        })
    })
    ...
    router.get('/emails/pb/email-forgotten-password', function (req, res) {
        ...
        var serviceName = req.param('serviceName')
        // re-render the page along with the parameter
        res.render('emails/pb/email-forgotten-password', {reason: theReason, vouch: vouched, service: service, serviceName: serviceName}, function(err, html) {
            res.send(html)
        })
    })
