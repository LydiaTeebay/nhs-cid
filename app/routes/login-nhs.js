module.exports = function (router) {
  router.get('/create-account/login-nhs', function (req, res) {
    ...
    router.get('/create-account/v10/login-nhs', function (req, res) {
        ...
        res.render('create-account/v10/login-nhs', { emailAddress: emailAddress, mobileNum: mobileNum, vouch: vouched, passwordChange: passwordChange, newUser: newUser, service: service, serviceName: serviceName, terms: terms, verified: verified, poluser: poluser }, function(err, html) {
            res.send(html)
        })
  ...
  router.get('/create-account/v7/login-nhs', function (req, res) {
    ...
    router.get('/delegated-access/login-nhs', function (req, res) {
        // pull in the url parameters
        var vouched = req.param('vouch')
        var service = req.param('service')
        var newUser = req.param('newUser')
        ...
        //    return
        //} else {
            res.render('delegated-access/login-nhs', { vouch: vouched, passwordChange: passwordChange, newUser: newUser, service: service, serviceName: serviceName, terms: terms }, function(err, html) {
                res.send(html)
            })
        //}
    })
