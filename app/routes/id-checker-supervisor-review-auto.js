module.exports = function (router) {
    router.get('/id-checker/id-checker-supervisor-review-auto', function (req, res) {
    ...
    router.get('/id-checker/v5/id-checker-supervisor-review-auto', function (req, res) {
        // pull in the url parameter
        var state = req.param('state')
        var queue = req.param('queue')
        var holding = req.param('holding')
        ...
        var demographics = req.param('demographics')
        var failreason = req.param('failreason')
        res.render('id-checker/v5/id-checker-supervisor-review-auto', {state: state, emailLink: emailLink, role: role, devMode: devmode, queue: queue, holding: holding, noabort: noabort, result: result, demographics: demographics, failreason: failreason }, function(err, html) {
            res.send(html)
        })
    })
}
